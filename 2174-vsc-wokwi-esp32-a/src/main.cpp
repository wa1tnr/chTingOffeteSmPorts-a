...
#include <Arduino.h>
#define TIME_STAMP "Tue 26 Aug 12:46:34 UTC 2025"
#define TSTAMP_VTAG " keer telfa murro"
...
void accep() {
  ...
  // TODO - restore - Serial.write(cData, len);
  top = len;
}
...
void great(void) { top = (stack[(unsigned char)S--] > top) LOGICAL; }
...
void slash(void) {
  top =
      (top) ? stack[(unsigned char)S--] / top : (stack[(unsigned char)S--], 0);
}
...
void adc(void) { top = (long)analogRead(top); }
...
void logoLocal() {
  Serial.println("");
  Serial.print("    PLEASE WAIT nine seconds .. still under program control..");
  delay(9345);
  Serial.println("");
  Serial.print(TIME_STAMP);
  Serial.println(TSTAMP_VTAG);
  Serial.println("");
  Serial.println(" the local logo on the half shell ");
  Serial.println("");
...
void loop() {
  ...
  while (true) {
    ...
    if (okFlag) {
      okFlag = false;
      // TODO - local modificiation - evaluate - Serial.print("  ");
      Serial.print("  ");
    }
    len = Serial.readBytes(cData, 256);
    // TODO - reinstate if wanted - char buffer[128]; buffer[0] = '\0';
    // buffer[1] = '\0';
    if (len) {
      // TODO - reinstate if wanted - snprintf(buffer, sizeof(buffer), "%s%c",
      // cData, '\0');
      // TODO - local modification - evaluate - Serial.print(buffer);
      break;
    }
  }
